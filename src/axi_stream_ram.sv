...
module axi_stream_ram #(
  parameter int unsigned             DEPTH         = 16,
  parameter int unsigned             DATA_WIDTH    = 16,
  parameter int unsigned             TAG_WIDTH     = 0,
  parameter logic [ DEPTH-1:0 ][ DATA_WIDTH-1:0 ] INITIAL_VALUE = '0,
  parameter string                   RAM_TYPE      = "auto"
) (
    input wire logic                        clk,
    input wire logic                        rst,
    // Write side
    output     logic                        wr_tready,
    input wire logic                        wr_tvalid,
    input wire logic [ $clog2(DEPTH)-1:0 ]  wr_addr,
    input wire logic [ DATA_WIDTH-1:0 ]     wr_data_in,
    output     logic [ DATA_WIDTH-1:0 ]     wr_data_out,

    // Read request side
    output     logic                        rd_in_tready,
    input wire logic                        rd_in_tvalid,
    input wire logic [ $clog2(DEPTH)-1:0 ]  rd_in_addr,
    input wire logic [ TAG_WIDTH-1:0 ]      rd_in_tag,

    // Read response side
    input wire logic                        rd_out_tready,
    output     logic                        rd_out_tvalid,
    output     logic [ $clog2(DEPTH)-1:0 ]  rd_out_addr,
    output     logic [ DATA_WIDTH-1:0 ]     rd_out_data,
    output     logic [ TAG_WIDTH-1:0 ]      rd_out_tag
);
