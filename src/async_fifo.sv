...
module async_fifo #(
  parameter int unsigned FIFO_LEN         = 512,
  parameter int unsigned UPPER_TRESHOLD   = 510,
  parameter int unsigned LOWER_TRESHOLD   = 10,
  parameter int unsigned DATA_WIDTH       = 8,
  parameter string       OVERFLOW_ACTION  = "SATURATE",
  parameter string       UNDERFLOW_ACTION = "SATURATE"
) (
    // Write port
    input wire logic                     wr_clk,
    input wire logic                     wr_arst,
    input wire logic [ DATA_WIDTH-1:0 ]  wr_data,
    input wire logic                     wr_en,
    output     logic                     wr_full,
    output     logic                     wr_upper,

    // Read port
    input wire logic                     rd_clk,
    input wire logic                     rd_arst,
    output     logic [ DATA_WIDTH-1:0 ]  rd_data,
    input wire logic                     rd_en,
    output     logic                     rd_dv,
    output     logic                     rd_lower,
