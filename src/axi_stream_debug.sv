...
module axi_stream_debug #(
  parameter int unsigned TDATA_WIDTH        = 32,
  parameter int unsigned TID_WIDTH          = 0,
  parameter int unsigned FRAME_LENGTH_WIDTH = 16,
  parameter int unsigned FRAME_COUNT_WIDTH  = 16
) (
    // Usual ports
    input wire logic                       clk,
    input wire logic                       rst,
    // Control and status
    // TODO: cfg/sts are the record types axi_stream_debug_cfg / axi_stream_debug_sts
    //       from interface_types_pkg. Convert that package to SV structs first, then
    //       replace these placeholders (widths use FRAME_LENGTH_WIDTH/FRAME_COUNT_WIDTH).
    // input  interface_types_pkg::axi_stream_debug_cfg_t  cfg,
    // output interface_types_pkg::axi_stream_debug_sts_t  sts,
    // AXI input
    output     logic                       s_tready,
    input wire logic                       s_tvalid,
    input wire logic                       s_tlast,
    input wire logic [ TDATA_WIDTH-1:0 ]   s_tdata,
    input wire logic [ TID_WIDTH-1:0 ]     s_tid,
    // AXI output
