...
module axi_stream_width_converter #(
  ...
  parameter int unsigned OUTPUT_DATA_WIDTH = 16,
  parameter int unsigned AXI_TID_WIDTH     = 0,
  parameter bit          IGNORE_TKEEP      = 1'b0
) (
    // Usual ports
    ...
    input wire logic [ INPUT_DATA_WIDTH-1:0 ]             s_tdata,
    input wire logic [ (INPUT_DATA_WIDTH+7)/8-1:0 ]       s_tkeep,
    input wire logic [ AXI_TID_WIDTH-1:0 ]                s_tid,
    input wire logic                                      s_tvalid,
    input wire logic                                      s_tlast,
    // AXI stream output
    input wire logic                                      m_tready,
    output     logic [ OUTPUT_DATA_WIDTH-1:0 ]            m_tdata,
    output     logic [ (OUTPUT_DATA_WIDTH+7)/8-1:0 ]      m_tkeep,
    output     logic [ AXI_TID_WIDTH-1:0 ]                m_tid,
